...
from service.resources.auth import LoginAPI
...
from service.middleware.auth import AuthMiddleware


class IdentityService(falcon.API):
    def __init__(self):

        cors = CORS(allow_all_origins=True)

        super(IdentityService, self).__init__(
            middleware=[cors.middleware, ContextMiddleware(), AuthMiddleware(), LogMiddleware()]
        )
...
def __find_resources__():
    """
    Find resources on the resources folder to be automatically loaded as APIs
    :note:
            In order to be loaded is advised to have the resources listed on all within the __init__.py
